...
namespace ZH.SocketModule.Sock
{
    ...
    internal class UDPClient : IConnection
    {
        ...
        public bool SendData(ref byte[] vData, bool IsReturn)
        {
            ...
            while (DateTime.Now.Subtract(Dt).TotalMilliseconds < MaxWaitSeconds)
            {
                Thread.Sleep(10);
                try
                {
        ...
        public bool UpdateBltSetting(string szSetting)
        {
            ...
            try
            {
                ...
                byte[] arr = Encoding.ASCII.GetBytes("reset");
                int sendlen = settingClient.Send(arr, arr.Length);

                Thread.Sleep(10);

                string str = "init " + szSetting.Replace(',', ' ');
                arr = Encoding.ASCII.GetBytes(str);
                sendlen = settingClient.Send(arr, arr.Length);
                settingClient.Close();
                return sendlen == arr.Length;
            }
            catch { }
            finally
            {
