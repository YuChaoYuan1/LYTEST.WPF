...
namespace LYTest.Mis.ZHGK_XinJiang
{
    public class MDS : IMis
    {
        ...
        public bool Down(string barcode, ref TestMeterInfo meter)
        {
            try
            {
                if (!stdDicCode.InitCode(out string Msg))
                {
                ...
                meter.MD_Factory = stdDicCode.GetCodeName("produceMf", meterDet.metMfr); //厂家;
        ...
        public bool Update(TestMeterInfo meter)
        {
            try
            {
                ...
                MDSResult.外观检查(itemResult, meter, Result, 不合格列表);
