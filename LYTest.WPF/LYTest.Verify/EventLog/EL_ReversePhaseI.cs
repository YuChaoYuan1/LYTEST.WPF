...
namespace LYTest.Verify.EventLog
{
    ...
    /// 电流逆相序事件记录
    /// </summary>
    class EL_ReversePhaseI : VerifyBase
    {

        string[] m_strEventStatus;
        /// <summary>
        /// 重写基类测试方法
        /// </summary>
        ...
        {      //获取电流值
            ...
            PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, xib, xib, xib, Cus_PowerYuanJian.H, 50, "1.0", Cus_PowerPhase.电流逆相序, PowerWay.正向有功);

            MessageAdd("电流逆相序事件", EnumLogType.提示信息);
            WaitTime("电流逆相序事件", 71);
            ...
            WaitTime("恢复电压并等待电流逆相序事件记录产生", 65);
        }
        ...
        private bool[] ReadEventLogInfo(int testNum, int happen)
        {
            ...
            if (Stop) return null;
            MessageAdd("读取电流逆相序记录总次数", EnumLogType.提示信息);
            string[] eventTimes = MeterProtocolAdapter.Instance.ReadData("电流逆相序总次数");
            ...
            for (int curNum = testNum; curNum >= 1; curNum--)
            {
                ...
                {
                    MessageAdd(string.Format("读取(上{0}次)电流逆相序记录开始时间", curNum), EnumLogType.提示信息);
                    timeStart = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次)电流逆相序发生时刻", curNum));

                    if (Stop) return null;
                    MessageAdd(string.Format("读取(上{0})次电流逆相序记录结束时间", curNum), EnumLogType.提示信息);
                    timeEnd = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次)电流逆相序结束时刻", curNum));
                }
                else if (OneMeterInfo.DgnProtocol.ClassName == "CDLT698")
                {
                    List<string> oad = new List<string> { "30100200" };
                ...
                for (int i = 0; i < MeterNumber; i++)
                {
                    if (!MeterInfo[i].YaoJianYn) continue;
                    string statu = "未电流逆相序";
                    int chr = m_strEventStatus[i].Length == 0 ? 0 : Convert.ToInt32(m_strEventStatus[i], 16);
                    if (OneMeterInfo.DgnProtocol.ClassName == "CDLT6452007")
                    { if ((chr & 0x02) == 0x02) statu = "电流逆相序"; }
                    else if (OneMeterInfo.DgnProtocol.ClassName == "CDLT698")
                    { if ((chr & 0x4000) == 0x4000) statu = "电流逆相序"; }



                    if (happen == 1)         //事件发生前
                    {
                        ResultDictionary["事件产生前事件状态"][i] = statu;
                    ...
                    {
                        ResultDictionary["事件产生后事件状态"][i] = statu;
