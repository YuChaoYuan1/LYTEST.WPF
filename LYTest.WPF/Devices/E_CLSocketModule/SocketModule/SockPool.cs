...
using System.Text;
using E_CLSocketModule.SocketModule.Sock;
using System.Net;
using System.Windows.Forms;
using System.Runtime.InteropServices;

namespace E_CLSocketModule.SocketModule
{
    /// <summary>
    ...
    public class SockPool
    {
        ...
        public static cl_Frontend_Regs.reg reg = new cl_Frontend_Regs.reg();
        ...
        public void AddUdpSock(string szSockName, IPAddress remoteIp, int remotePort, int localPort, int BasePort, string HaveProtocol, int MaxWaitMSecond, int WaitSecondPerByte)
        {
            if (iVer == 0)
            {
                //reg.sName = "90378612";
                //if (reg.BoolCode(reg.CreateCode(), g_GetINI(Application.StartupPath + "\\System\\VirtualMeter.ini", "System", "Registered", "698.45")) != "czx")
                //{
            ...
            lock (objLock)
            {
                ...
                Connection s = new Connection(remoteIp, remotePort, localPort, BasePort, HaveProtocol, MaxWaitMSecond, WaitSecondPerByte);
        ...
        /// <param name="recvPacket">接收包</param>
        /// <returns>发送是否成功</returns>
        public bool Send(string szSockName, Packet.SendPacket sendPacket, Packet.RecvPacket recvPacket, int MaxWaitSeconds)
        {
            bool ret = false;
            ...
            if (dicLock.ContainsKey(szSockName))
            {
                lock (dicLock[szSockName])
                {
                    ...
                    ret = DicSock[szSockName].Send(sendPacket, recvPacket, szSockName, MaxWaitSeconds);
