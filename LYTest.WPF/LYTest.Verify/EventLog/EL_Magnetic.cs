...
namespace LYTest.Verify.EventLog
{
    ...
    class EL_Magnetic : VerifyBase
    {
        ...
        {
            Identity();
            MessageAdd("恒定磁场干扰事件", EnumLogType.提示信息);
            ...
            WaitTime( "恒定磁场干扰事件",61);

            MessageAdd("用户进行进行恒定磁场干扰...", EnumLogType.提示信息);
            MessageBox.Show("请进行恒定磁场干扰操作\n操作完成后请按确定键", "恒定磁场干扰事件", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private string[] ReadEventLogInfo(int happen)
        {
            ...
            if (Stop) return null;
            MessageAdd("读取电表恒定磁场干扰事件总次数", EnumLogType.提示信息);
            string[] eventTimes = MeterProtocolAdapter.Instance.ReadData("电能表恒定磁场干扰总次数");

            if (Stop) return null;

            MessageAdd("读取上1次恒定磁场干扰记录", EnumLogType.提示信息);
            ...
            {
                List<string> oad = new List<string> { "302A0200" };
