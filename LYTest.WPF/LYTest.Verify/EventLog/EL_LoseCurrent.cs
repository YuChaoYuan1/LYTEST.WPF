...
namespace LYTest.Verify.EventLog
{
    ...
    /// 失流事件记录
    /// </summary>
    class EL_LoseCurrent : VerifyBase
    {
        ...
        private int[] ReadEventLogInfo2(Cus_PowerYuanJian yuanJian, string name)
        {
            ...
            MessageAdd("读取失流记录总次数", EnumLogType.提示信息);
            string[] eventTimes = MeterProtocolAdapter.Instance.ReadData(yuanJian.ToString() + "相失流总次数");
            ...
            {
                MessageAdd("失流记录开始时间", EnumLogType.提示信息);
                eventTimeStart = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}相失流产生时刻", 1, yuanJian.ToString()));

                if (Stop) return null;
                MessageAdd(string.Format("读取上【{0:D2}】次【{1}】相失流记录结束时间", 1, yuanJian.ToString()), EnumLogType.提示信息);
                eventTimeEnd = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}相失流结束时刻", 1, yuanJian.ToString()));
            }
            else
            {
                MessageAdd(string.Format("读取上【{0:D2}】次【{1}】失流记录", 1, yuanJian.ToString()), EnumLogType.提示信息);
        ...
        private void StartEventLog(Cus_PowerYuanJian yuanJian)
        {
            ...
            switch (yuanJian)
            {
                case Cus_PowerYuanJian.B:
                    ...
                    msg = "读取电表运行状态字5（B相故障状态）";
                    dataFlag = "电表运行状态字4";
                    PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB * 0.75F, OneMeterInfo.MD_UB, xib, xib * 0.004f, xib, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                ...
                case Cus_PowerYuanJian.C:
                    ...
                    msg = "读取电表运行状态字6（C相故障状态）";
                    dataFlag = "电表运行状态字4";
                    PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, xib, xib, xib * 0.004f, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
                default:
                    msg1 = "A相失流事件";
                    msg = "读取电表运行状态字4（A相故障状态）";
                    dataFlag = "电表运行状态字4";
                    PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, xib * 0.004f, xib, xib, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
            }
            MessageAdd(msg1, EnumLogType.提示信息);
