...
namespace LYTest.DataManager.SG
{
    public class SearchMenuItem : ViewModelBase
    {
        ...
        public bool IsNumber
        {
            get
            {
                return new List<string>() { "METER_ID", "MD_EPITOPE" }.Contains(fieldName);
            }
        }
        ...
        /// 数字正则表达式
        /// </summary>
        private Regex NumberRegex { get; set; } = new Regex(@"[-+]?\d+(.\d+)?");
        /// <summary>
        /// 比较字符串
        /// </summary>
        /// <returns></returns>
        public override string ToString()
        {
            ...
            string valueTemp = "";
            if (ValueType == EnumValueType.编码值)
            {
            ...
            {
                #region 数字格式
                Match match=NumberRegex.Match(realValue);
                double.TryParse(match.Value, out double doubleResult);
                realValue=doubleResult.ToString();
                ...
                switch (CompareExpression)
                {
                    ...
                    case EnumCompare.自定义筛选条件:
                        ...
                        break;
                    case EnumCompare.大于等于:
                        stringTemp = string.Format("{0} >= {1}", FieldName, realValue);
                        break;
                    case EnumCompare.小于等于:
                        stringTemp = string.Format("{0} <= {1}", FieldName, realValue);
                        break;
                }
                #endregion
