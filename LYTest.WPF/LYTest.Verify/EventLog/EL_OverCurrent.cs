...
namespace LYTest.Verify.EventLog
{
    ...
    /// 过流事件记录
    /// </summary>
    class EL_OverCurrent : VerifyBase
    {
        ...
        public override void Verify()
        {
            ...
            try
            {
                if (DAL.Config.ConfigHelper.Instance.IsITOMeter)
                {
                    ...
                    dicEnergy = MeterProtocolAdapter.Instance.ReadData("电能表过流事件_配置参数");
                    OverReslut = MeterProtocolAdapter.Instance.WriteData("电能表过流事件_配置参数", OverPower + "|" + OverTime);
                }
            ...
            finally
            {
                ...
                if (DAL.Config.ConfigHelper.Instance.IsITOMeter)
                {
                    MessageAdd("恢复电能表过流事件参数", EnumLogType.提示信息);
                    for (int i = 0; i < OverReslut.Length - 1; i++)
                    {
                        if (OverReslut[i])
                        {
                            MeterProtocolAdapter.Instance.WriteData("电能表过流事件_配置参数", dicEnergy[i]);
                            break;
                        }
                    }
                    MessageAdd("恢复电能表过流事件参数完成", EnumLogType.提示信息);
        ...
        private int[] ReadEventLogInfo2(Cus_PowerYuanJian yuanJian, string name)
        {
            ...
            MessageAdd("读取过流记录总次数", EnumLogType.提示信息);
            string[] eventTimes = MeterProtocolAdapter.Instance.ReadData(yuanJian.ToString() + "相过流总次数");
            ...
            {
                MessageAdd("过流记录开始时间", EnumLogType.提示信息);
                eventTimeStart = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}相过流产生时刻", 1, yuanJian.ToString()));

                if (Stop) return null;
                MessageAdd(string.Format("读取上【{0:D2}】次【{1}】相过流记录结束时间", 1, yuanJian.ToString()), EnumLogType.提示信息);
                eventTimeEnd = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}相过流结束时刻", 1, yuanJian.ToString()));
            }
            else
            {
                MessageAdd(string.Format("读取上【{0:D2}】次【{1}】过流记录", 1, yuanJian.ToString()), EnumLogType.提示信息);
            ...
            for (int j = 0; j < MeterNumber; j++)
            {
                ...
                if (OneMeterInfo.DgnProtocol.ClassName == "CDLT6452007" && (chr & 0x01) == 0x01)
                    statu = "过流";
                else if (OneMeterInfo.DgnProtocol.ClassName == "CDLT698" && (chr & 0x0800) == 0x0800)
                    statu = "过流";
        ...
        private void StartEventLog(Cus_PowerYuanJian yuanJian)
        {
            ...
            switch (yuanJian)
            {
                case Cus_PowerYuanJian.B:
                    ...
                    dataFlag = "电表运行状态字5";
                    PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, xib, float.Parse((xmax * 1.2f).ToString("0.0")), xib, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
                case Cus_PowerYuanJian.C:
                    eventMsg = "C相过流事件";
                    msg = "读取电表运行状态字6（C相故障状态）";
                    dataFlag = "电表运行状态字6";
                    PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, xib, xib, float.Parse((xmax * 1.2f).ToString("0.0")), Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
                default:
                    eventMsg = "A相过流事件";
                    msg = "读取电表运行状态字4（A相故障状态）";
                    dataFlag = "电表运行状态字4";
                    PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, float.Parse((xmax * 1.2f).ToString("0.0")), xib, xib, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
            }
        ...
        //    if (Stop) return null;
        //    MessageAdd(string.Format("读取【{0}】过流记录总次数", strPhase), EnumLogType.提示信息);
        //    string[] eventTimes = MeterProtocolAdapter.Instance.ReadData(string.Format("{0}过流总次数", strPhase));
        ...
        //        if (OneMeterInfo.DgnProtocol.ClassName == "CDLT6452007")
        //        {
        //            MessageAdd(string.Format("读取(上{0}次){1}过流记录开始时间", curNum, strPhase), EnumLogType.提示信息);
        //            startTime = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}失流发生时刻", curNum, strPhase));

        //            if (Stop) return null;
        //            MessageAdd(string.Format("读取(上{0}次){1}过流记录结束时间", curNum, strPhase), EnumLogType.提示信息);
        //            endTime = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}失流结束时刻", curNum, strPhase));
        //        }
        //        else if (OneMeterInfo.DgnProtocol.ClassName == "CDLT698")
        //        {
        //            MessageAdd(string.Format("读取(上{0}次){1}过流记录", curNum, strPhase), EnumLogType.提示信息);
        //            //List<string> oad = new List<string> { "30050700" };
        //            List<string> oad = new List<string>();  //3000-电能表欠压事件，07-A相事件记录表
        //            if (strPhase == "B相")
        //                oad.Add("30050800");
        //            else if (strPhase == "C相")
        //                oad.Add("30050900");
        ...
        //            int chr = m_strEventStatus[j].Length == 0 ? 0 : Convert.ToInt32(m_strEventStatus[j], 16);
        //            if (OneMeterInfo.DgnProtocol.ClassName == "CDLT6452007")
        //            { if ((chr & 0x10) == 0x10) statu = "过流"; }
        //            else if (OneMeterInfo.DgnProtocol.ClassName == "CDLT698")
        //            { if ((chr & 0x800) == 0x800) statu = "过流"; }

        //            if (happen == 1)         //事件发生前
        //            {
        //                ResultDictionary[strPhase + "事件产生前事件状态"][j] = statu;
        //                ResultDictionary[strPhase + "事件产生前事件发生时刻"][j] = startTime[j];
        //                ResultDictionary[strPhase + "事件产生前事件结束时刻"][j] = endTime[j];
        //                ResultDictionary[strPhase + "事件产生前总次数"][j] = eventTimes[j];
        //            }
        //            else if (happen == 2)    //事件发生后
        //            {
        //                ResultDictionary[strPhase + "事件产生后事件状态"][j] = statu;
        //                ResultDictionary[strPhase + "事件产生后事件发生时刻"][j] = startTime[j];
        //                ResultDictionary[strPhase + "事件产生后事件结束时刻"][j] = endTime[j];
        //                ResultDictionary[strPhase + "事件产生后总次数"][j] = eventTimes[j];
        ...
        //        if (happen == 1)
        //        {
        //            RefUIData(strPhase + "事件产生前事件状态");
        //            RefUIData(strPhase + "事件产生前事件发生时刻");
        //            RefUIData(strPhase + "事件产生前事件结束时刻");
        //            RefUIData(strPhase + "事件产生前总次数");
        //        }
        //        else
        //        {
        //            RefUIData(strPhase + "事件产生后事件状态");
        //            RefUIData(strPhase + "事件产生后事件发生时刻");
        //            RefUIData(strPhase + "事件产生后事件结束时刻");
        //            RefUIData(strPhase + "事件产生后总次数");
        //        }
        //    }
        //    return bResult;
        //}
