...
namespace E_ZH3001
{
    ...
    #region ZH3001设置2103H ---- 设置启动或停止电能误差计算
    internal class ZH3001_RequestSetErrorDataPacket : ZH3001SendPacket
    {
        public int _StopOrstart = 0;
        private int _TypePush = 0;
        private int _ConstMetet = 1200;
        private int _QS = 2;

        public void SetPara(int StopOrstart, int TypePush, int ConstMetet, int QS)
        {
            _StopOrstart = StopOrstart;
            _TypePush = TypePush;
            _ConstMetet = ConstMetet;
            _QS = QS;
        }
        ...
        protected override byte[] GetBody()
        {
            ...
            buf.Put(0x21);
            buf.Put(0x03);
            buf.Put(Convert.ToByte(_StopOrstart));
            buf.Put(Convert.ToByte(_TypePush));
            buf.PutInt(_ConstMetet);
            buf.PutInt(_QS);
            return buf.ToByteArray();
        }
        ...
        public override string GetPacketResolving()
        {
            return $"设置误差参数：{_StopOrstart},{_TypePush},{_ConstMetet},{_QS}";
        }
    }

    internal class ZH3001_RequestSetErrorDataReplyPacket : ZH3001RecvPacket
    {
        protected override void ParseBody(byte[] data)
        {
            if (data == null || data.Length != 4)
        ...
        public override string GetPacketResolving()
        {
            return $"返回：{ReciveResult}";
        }
    }
    #endregion 发送ZH3001设置2100H----设置负载电流快速变化实验的时间与启动标志

    #region ZH3001 读取2104H ---- 读取电能误差计算结果
    internal class ZH3001_RequestReadErrorDataPacket : ZH3001SendPacket
    {
    ...
    internal class ZH3001_RequestReadErrorDataReplyPacket : ZH3001RecvPacket
    {
        public float[] Data { get; set; } = new float[12];
        protected override void ParseBody(byte[] data)
        {
            ...
            else
            {
                if (data[0] == 0x90)
                {
                    for (int i = 0; i < Data.Length; i++)
                    {
                        ...
                        for (int j = 0; j < 4; j++)
                        {
                            strTemp += data[i * 4 + j + 3].ToString("X2");
                        }
                        float Fdbs = 1000f;
                        if (i == 0 || i == 4 || i == 8)
                        {
                        ...
                        Data[i] = Convert.ToInt32((strTemp), 16) / Fdbs;
                    }
                    ReciveResult = RecvResult.OK;
    ...
    #region ZH3001 读取100AH ---- 读取常数
    internal class ZH3001_RequestReadContantPacket : ZH3001SendPacket
    {
    ...
    internal class ZH3001_RequestReadContantReplyPacket : ZH3001RecvPacket
    {
        ...
        protected override void ParseBody(byte[] data)
        {
            ...
            else
            {
                if (data[0] == 0x90)
                {
                    ...
                    for (int j = 1; j <= 4; j++)
                    {
                        tmp += data[j].ToString("X2");
                    }
                    Data = Convert.ToInt32(tmp, 16);
                    ReciveResult = RecvResult.OK;
    ...
    internal class ZH3001_RequestPowerGetHarmonicPacket : ZH3001SendPacket
    {
        ...
        protected override byte[] GetBody()
        {
            ...
            //str_Temp = "0";
            //_ = int.Parse(str_Temp).ToString("X8") + "FC";
    ...
    /// 间谐波 请求包
    /// </summary>
    internal class ZH3001_RequestPowerInterharmonicsPacket : ZH3001SendPacket
    {

        //谐波输出形式：1个字节表示，00表示正常谐波，01表示间谐波输出；
        //间谐波输出为：1.5次，2.5次……63.5次。配合2008H设置谐波参数寄存器使用，先下发2109H指令后，再下发2008H的指令。
        //指令：68 01 01 09 13 21 09 01  33
        //返回：68 01 01 09 93 21 09



        public byte _HarmonicType = 0x00;
        /// <summary>
        /// 00普通谐波，01间谐波
        /// </summary>
        /// <param name="HarmonicType"></param>
        public void SetPara(byte HarmonicType)
        {
            _HarmonicType = HarmonicType;
        }
        protected override byte[] GetBody()
    ...
    /// 间谐波 返回指令
    /// </summary>
    internal class ZH3001_RequestPowerInterharmonicsReplyPacket : ZH3001RecvPacket
    {
        protected override void ParseBody(byte[] data)
        {
            if (data == null || data.Length != 4)
    ...
    #endregion
    #region  电能质量相关的电压实验项目
    /// <summary>
    /// 电能质量
    /// </summary>
    internal class ZH3001_RequestPowerQualityPacket : ZH3001SendPacket
    {
        //        试验类型：1个字节表示，00H是电压波动，01H是电压闪变；
        //试验项目：1个字节表示，
        //				试验类型为电压波动时：参数为1、2、3（非这三个数值无效）分别对应电压波动试验项目中的检测点（1）、（2）、（3）
        //				试验项目为电压闪变时：参数为1、3（非这两个数值无效）分别对应闪变试验中的短时闪变值为1和闪变值为3的试验点。
        //使能状态：1个字节表示，01H表示启动，其他无效，停止时只需要下发关源指令即可。
        //指令如：68 01 01 0B 13 22 00 00 01 01 3A
        //返回：68 01 01 09 93 22 00 4B F3


        /// <summary>
        /// 1个字节表示，00H是电压波动，01H是电压闪变
        /// </summary>
        public byte _QualityType = 0x00;
        /// <summary>
        ///  1个字节表示，试验类型为电压波动时：参数为1、2、3（非这三个数值无效）分别对应电压波动试验项目中的检测点（1）、（2）、（3）
        ///  试验项目为电压闪变时：参数为1、3（非这两个数值无效）分别对应闪变试验中的短时闪变值为1和闪变值为3的试验点。
        /// </summary>
        public byte _Model = 0x01;
        /// <summary>
        ///  使能状态：1个字节表示，01H表示启动，其他无效，停止时只需要下发关源指令即可
        /// </summary>
        public byte _State = 0x01;
        ...
        public void SetPara(byte QualityType, byte Model, byte State)
        {
    ...
    /// 电能质量 返回指令
    /// </summary>
    internal class ZH3001_RequestPowerQualityReplyPacket : ZH3001RecvPacket
    {
        protected override void ParseBody(byte[] data)
        {
            if (data == null || data.Length != 4)
