...
namespace LYTest.Verify.EventLog
{
    ...
    class EL_LoseVoltage : VerifyBase
    {
        ...
        private int[] ReadEventLogInfo2(Cus_PowerYuanJian yuanJian, string name)
        {
            ...
            MessageAdd("读取失压记录总次数", EnumLogType.提示信息);
            string[] eventTimes = MeterProtocolAdapter.Instance.ReadData(yuanJian.ToString() + "相失压总次数");
            ...
            {
                MessageAdd("失压记录开始时间", EnumLogType.提示信息);
                eventTimeStart = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}相失压产生时刻", 1, yuanJian.ToString()));

                if (Stop) return null;
                MessageAdd(string.Format("读取上【{0:D2}】次【{1}】相失压记录结束时间", 1, yuanJian.ToString()), EnumLogType.提示信息);
                eventTimeEnd = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}相失压结束时刻", 1, yuanJian.ToString()));
            }
            else
            {
                MessageAdd(string.Format("读取上【{0:D2}】次【{1}】失压记录", 1, yuanJian.ToString()), EnumLogType.提示信息);
        ...
        /// 开始切换
        /// </summary>
        /// <param name="phase">1-A相，2-B相，3-C相</param>
        /// <returns></returns>
        private void StartEventLog(Cus_PowerYuanJian yuanJian)
        {
            ...
            switch (yuanJian)
            {
                case Cus_PowerYuanJian.B:
                    ...
                    dataFlag = "电表运行状态字5";
                    PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB * 0.2F, OneMeterInfo.MD_UB, xib * 0.06F, xib * 0.06f, xib * 0.06F, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
                case Cus_PowerYuanJian.C:
                    eventMsg = "C相失压事件";
                    msg = "读取电表运行状态字6（C相故障状态）";
                    dataFlag = "电表运行状态字6";
                    PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, OneMeterInfo.MD_UB * 0.2F, xib * 0.06F, xib * 0.06f, xib * 0.06F, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
                default: //1或其它
                    eventMsg = "A相失压事件";
                    msg = "读取电表运行状态字4（A相故障状态）";
                    dataFlag = "电表运行状态字4";
                    PowerOn(OneMeterInfo.MD_UB * 0.5F, OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, xib * 0.06F, xib * 0.06f, xib * 0.06F, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
            }
