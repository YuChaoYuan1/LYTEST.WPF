...
namespace LYTest.Verify.AccurateTest
{
    ...
    /// 谐波电压
    /// </summary>
    /// 结论数据：基波电压|基波频率|谐波含有率|谐波次数|允许误差|测量值|相对误差
    /// 方案参数：谐波电压含有率
    public class HarmonicVoltage : VerifyBase
    {
        float 谐波含量 = 0f;
        ...
        public override void Verify()
        {
            ...
            ReadData.Add(MeterProtocolAdapter.Instance.ReadData("A相谐波电压含有率"));
            Names.Add("A");
            if (OneMeterInfo.MD_WiringMode == "三相三线")
            {
                ReadData.Add(MeterProtocolAdapter.Instance.ReadData("C相谐波电压含有率"));
                Names.Add("C");
            }
            else
            {
                ReadData.Add(MeterProtocolAdapter.Instance.ReadData("B相谐波电压含有率"));
                ReadData.Add(MeterProtocolAdapter.Instance.ReadData("C相谐波电压含有率"));
                Names.Add("B");
                Names.Add("C");
            }
            ...
            for (int j = 0; j < MeterNumber; j++)
            {
                ...
                foreach (var item in 谐波次数)
                {
                    ...
                    for (int i = 0; i < values.Count; i++)        //循环ABC三相
                    {
                        ...
                        {
                            case "A":
                                stdIndex = (item - 1) * 6; //标准表谐波电压的，谐波电流全部+1
                                break;
                            case "B":
                                stdIndex = (item - 1) * 6 + 2;
                                break;
        ...
        protected override bool CheckPara()
        {
            ...
            ResultNames = new string[] { "基波电压", "基波频率", "谐波含有率", "谐波次数", "允许误差", "相对误差", "测量值", "结论" };
            return true;
        }
    }
}
