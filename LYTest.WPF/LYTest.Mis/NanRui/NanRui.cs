...
using LYTest.Mis.NanRui.Tables;
...
//add yjt 20220222 新增整个南瑞MDS接口（整个NanRui文件夹）
namespace LYTest.Mis.NanRui
{
    public class NanRui : OracleHelper, IMis
    {
        public string SysNo { get; private set; } = "450";
        public NanRui(string ip, int port, string dataSource, string userId, string pwd, string url, string sysno)
        {
            this.Ip = ip;
            this.Port = port;
        ...
        /// <returns></returns>
        public List<MT_METER_POSI> GetList(string taskNo, string equipmentNo)
        {
            if (taskNo == null)
            {
                return new List<MT_METER_POSI>();
            }
            List<MT_METER_POSI> meters = new List<MT_METER_POSI>();

            string sqlStr = string.Format("select * from MT_METER_POSI  where  MT_METER_POSI.DETECT_TASK_NO='{0}' and MT_METER_POSI.EQUIP_NO='{1}'", taskNo.Trim(), equipmentNo);
            DataTable dataTable = ExecuteReader(sqlStr);
            foreach (DataRow reader in dataTable.Rows)
            {
                MT_METER_POSI meter = new MT_METER_POSI
                {
                    DETECT_TASK_NO = reader["DETECT_TASK_NO"].ToString(),
                    BAR_CODE = reader["BAR_CODE"].ToString(),
                    EQUIP_NO = reader["EQUIP_NO"].ToString(),
                    EQUIP_INDX = reader["EQUIP_INDX"].ToString(),
                    WRITE_DATE = reader["WRITE_DATE"].ToString(),
                    HANDLE_FLAG = reader["HANDLE_FLAG"].ToString(),
                    HANDLE_DATE = reader["HANDLE_DATE"].ToString()
                };
                if (string.IsNullOrWhiteSpace(meter.BAR_CODE))
                {
                    continue;
                }
                meters.Add(meter);
        ...
        public bool Update(TestMeterInfo meterInfo)
        {
            ...
            {
                sqlrst = false;
                LogManager.AddMessage($"外观检查试验插入出现问题:{ex.Message}", EnumLogSource.检定业务日志, EnumLevel.Error);
            }

            try
            {
                //启动
                if (false == GetMT_STARTING_MET_CONCByMt(meter, meterInfo, out string[] outSql, out string err))
                {
                    LogManager.AddMessage($"数据上传失败:{err}", EnumLogSource.检定业务日志, EnumLevel.Error);
                    return false;
                }

                foreach (string str in outSql)
                {
                    sqlList.Add(str);
                }
                //潜动
                if (false == GetMT_CREEPING_MET_CONCByMt(meter, meterInfo, out string[] outSql1, out string err1))
                {
                    LogManager.AddMessage($"数据上传失败:{err1}", EnumLogSource.检定业务日志, EnumLevel.Error);
                    return false;
                }
                foreach (string str in outSql1)
                {
                    sqlList.Add(str);
                }
            }
            ...
            {
                sqlrst = false;
                LogManager.AddMessage("启动潜动试验插入出现问题" + ex, EnumLogSource.检定业务日志, EnumLevel.Error);
            }
            ...
            catch (Exception ex)
            {
                ...
                LogManager.AddMessage($"常数试验插入出现问题:{ex.Message}", EnumLogSource.检定业务日志, EnumLevel.Error);
                LogManager.AddMessage(ex.StackTrace, EnumLogSource.检定业务日志, EnumLevel.Error);
            }

            try
            {
                //基本误差试验
                if (false == GetMT_BASICERR_MET_CONCByMt(meter, meterInfo, out string[] outSql, out string err))
                {
                    LogManager.AddMessage($"数据上传失败:{err}", EnumLogSource.检定业务日志, EnumLevel.Error);
                    return false;

                }
                sqlList.AddRange(outSql);

                //（初始固有误差表）基本误差试验
                sql = GetMT_INIERR_MET_CONCByMt(meter, meterInfo);
                if (sql != null)
                {
                    sqlList.AddRange(sql);
                }
            }
            catch (Exception ex)
            {
                sqlrst = false;
                LogManager.AddMessage("基本误差和标准偏差（重复性）实验插入出现问题" + ex, EnumLogSource.检定业务日志, EnumLevel.Error);
            }
            ...
            {
                sqlrst = false;
                LogManager.AddMessage("日计时试验插入出现问题" + ex, EnumLogSource.检定业务日志, EnumLevel.Error);
            }
            ...
            {
                sqlrst = false;
                LogManager.AddMessage("时间误差试验插入出现问题" + ex, EnumLogSource.检定业务日志, EnumLevel.Error);
            }
            ...
            {
                sqlrst = false;
                LogManager.AddMessage("计度器总电能示值组合误差试验插入出现问题" + ex, EnumLogSource.检定业务日志, EnumLevel.Error);
            }
            ...
            {
                sqlrst = false;
                LogManager.AddMessage("需量示值误差试验插入出现问题" + ex, EnumLogSource.检定业务日志, EnumLevel.Error);
            }
            ...
            {
                sqlrst = false;
                LogManager.AddMessage("规约一致性试验插入出现问题" + ex, EnumLogSource.检定业务日志, EnumLevel.Error);
            }
            ...
            {
                sqlrst = false;
                LogManager.AddMessage("耐压试验插入出现问题" + ex, EnumLogSource.检定业务日志, EnumLevel.Error);
            }
            ...
            {
                sqlrst = false;
                LogManager.AddMessage("总结论插入出现问题" + ex, EnumLogSource.检定业务日志, EnumLevel.Error);
            }
        ...
        public bool SchemeDownDetPara(string where, ref Dictionary<string, SchemaNode> Schema)
        {
            ...
            for (int j = 0; j < det.Rows.Count; j++)
            {
                ...
                {
                    LogManager.AddMessage($"MT_DETECT_PARA_INFO表中不存在{where}  para_id='{PARA_ID}')的方案信息", Utility.Log.EnumLogSource.数据库存取日志);
                    return false;
                }
        ...
        private MT_METER GetMeter(string barcode)
        {
            ...
            meter.MT_DATECT_OUT_EQUIP.SYS_NO = row["SYS_NO"].ToString();
            meter.MT_DATECT_OUT_EQUIP.DETECT_TASK_NO = row["DETECT_TASK_NO"].ToString();
            return meter;
        }
        ...
        /// 启动
        /// </summary>
        protected bool GetMT_STARTING_MET_CONCByMt(MT_METER mtMeter, TestMeterInfo meter, out string[] outSqls, out string err)
        {
            outSqls = Array.Empty<string>();
            err = "";
            ...
            foreach (string key in meterQdQids.Keys)
            {
                ...
                string current = curtmp.ToString("F4");// + "Ib";
                ...
                if (meter.MD_JJGC == "IR46")
                {
                    ...
                    {
                        if (meter.MD_ConnectionFlag.IndexOf("直接") >= 0)
                            loadcstr = loadc * 10 + "Itr";
                        else
                            loadcstr = loadc * 20 + "Itr";
                    }
                }
                else
                {
                    ...
                    else
                    {
                        if (meter.MD_ConnectionFlag.IndexOf("直接") >= 0)
                            ...
                            loadcstr = Math.Round(loadc / 20, 3).ToString().TrimEnd('0') + "Ib";
                    }
                }
                ...
                MT_STARTING_MET_CONC entity = new MT_STARTING_MET_CONC
                {
                    ...
                    START_CURRENT = current,// GetPCode("meter_Test_CurLoad", loadcstr),//current,
                };

                //if (string.IsNullOrEmpty(entity.BOTH_WAY_POWER_FLAG) || string.IsNullOrEmpty(entity.LOAD_CURRENT))
                if (string.IsNullOrEmpty(entity.BOTH_WAY_POWER_FLAG) || string.IsNullOrEmpty(entity.START_CURRENT))
                {
                    err = "启动试验的功率方向或电流值不能为空";
                    return false;
                }
        ...
        /// 潜动
        /// </summary>
        protected bool GetMT_CREEPING_MET_CONCByMt(MT_METER mtMeter, TestMeterInfo meter, out string[] outSqls, out string err)
        {
            outSqls = Array.Empty<string>();
            err = "";
            int index = 1;
            ...
            foreach (string key in meterQdQids.Keys)
            {
                if (key.Split('_')[0] != ProjectID.潜动试验) continue;
                ...
                if (float.TryParse(meterQdQids[key].Voltage, out float vtmp))
                {
                    float vRate = vtmp / meter.MD_UB * 100;
                    entity.LOAD_VOLTAGE = GetPCode("meterTestVolt", $"{vRate:f0}%Un");
                }

                if (string.IsNullOrEmpty(entity.BOTH_WAY_POWER_FLAG) || string.IsNullOrEmpty(entity.LOAD_VOLTAGE))
                {
                    err = "潜动试验的功率方向或电压值不能为空";
                    return false;
                }
        ...
        protected string[] GetMT_CONST_MET_CONCByMt(MT_METER mtMeter, TestMeterInfo meter)
        {
            ...
            string[] sqls = new string[needUpCode.Count];
            for (int i = 0; i < needUpCode.Count; i++)
            {
                ...
                //LogHelper.WriteLog($"D_{i}_D");
                sqls[i] = entity.ToInsertString();
            }
        ...
        /// 基本误差数据
        /// </summary>
        protected bool GetMT_BASICERR_MET_CONCByMt(MT_METER mtMeter, TestMeterInfo meter, out string[] outSqls, out string err)
        {
            outSqls = Array.Empty<string>();
            err = "";
            List<string> sql = new List<string>();
            ...
            for (int i = 0; i < keys.Length; i++)
            {
                ...
                MT_BASICERR_MET_CONC entity = new MT_BASICERR_MET_CONC
                {
                    ...
                    DETECT_ITEM_POINT = (i + 1).ToString(),
                    IS_VALID = "1",
                    BOTH_WAY_POWER_FLAG = GetPCode("powerFlag", meterErr.GLFX)
                };
                ...
                if (string.IsNullOrEmpty(entity.BOTH_WAY_POWER_FLAG) || string.IsNullOrEmpty(entity.PF) || string.IsNullOrEmpty(entity.LOAD_CURRENT))
                {
                    err = "基本误差试验的功率方向或电流值或功率因数不能为空";
                    return false;
                }
        ...
        /// 初始固有误差
        /// </summary>
        protected string[] GetMT_INIERR_MET_CONCByMt(MT_METER mtMeter, TestMeterInfo meter)
        {
            List<string> sql = new List<string>();
            string[] keys = new string[meter.MeterErrors.Keys.Count];
            meter.MeterErrors.Keys.CopyTo(keys, 0);
            ...
            {
                string key = keys[i];
                if (!key.StartsWith("12015")) continue;
                //if (key.Substring(0, 1) == "2") continue;
                ...
                MT_INIERR_MET_CONC entity = new MT_INIERR_MET_CONC
                {
                    DETECT_TASK_NO = mtMeter.MT_DATECT_OUT_EQUIP.DETECT_TASK_NO,
                    EQUIP_CATEG = "01",
                    SYS_NO = mtMeter.MT_DATECT_OUT_EQUIP.SYS_NO,
        ...
        protected string GetMT_CLOCK_VALUE_MET_CONC(MT_METER mtMeter, TestMeterInfo meter)
        {
            ...
            if (meter.MeterDgns.ContainsKey(ItemKey))
            {
                ...
                if (value.Length == 3)
                {
                    meterTime = value[0]?.Replace("/", "-");
                    stdTime = value[1]?.Replace("/", "-");
                    wc = value[2];
                }
            }
            ...
            MT_CLOCK_VALUE_MET_CONC entity = new MT_CLOCK_VALUE_MET_CONC
            {
                ...
                SYS_NO = mtMeter.MT_DATECT_OUT_EQUIP.SYS_NO,
                DETECT_EQUIP_NO = meter.BenthNo,
                DETECT_UNIT_NO = meter.BenthNo,//string.Empty,
                POSITION_NO = meter.MD_Epitope.ToString(),
                BAR_CODE = mtMeter.BAR_CODE,
                DETECT_DATE = Convert.ToDateTime(meter.VerifyDate).ToString("yyyy-MM-dd HH:mm:ss"),
                PARA_INDEX = "1",
        ...
        protected string[] GetMT_HUTCHISON_COMBINA_MET_CONCByMt(MT_METER mtMeter, TestMeterInfo meter)
        {
            ...
            MT_HUTCHISON_COMBINA_MET_CONC entity = new MT_HUTCHISON_COMBINA_MET_CONC
            {
                ...
                ERR_DOWN = "-0.01",
                ERR_UP = "0.01",
                VOLTAGE = GetPCode("meterTestVolt", "100%Un"),
                TOTAL_READING_ERR = "",
                TOTAL_INCREMENT = "",
                SUMER_ALL_INCREMENT = "",
                SHARP_INCREMENT = "",
            ...
            if (meter.MeterDgns.ContainsKey(key))
            {
                if (value.Length > 5)
                {
                    //float.TryParse(value[0], out incstart);
                    //float.TryParse(value[1], out incend);
        ...
        protected string[] GetMT_TS_MET_CONC(MT_METER mtMeter, TestMeterInfo meter)
        {
            ...
            if (meter.MeterDgns.ContainsKey(ItemKey))
            {
                ...
                {
                    entity.DETECT_TASK_NO = mtMeter.MT_DATECT_OUT_EQUIP.DETECT_TASK_NO;
                ...
                if (value.Length >= 3)
                {
                    ...
                    for (int i = 0; i < testvalueone.Length; i++)
                    {
                        ...
                        if (i >= BZvalue.Length || i >= SJvalue.Length || i >= valueErr.Length || para.Length < 2)
                            continue;
                        entity.FEE = GetPCode("tari_ff", para[1].TrimEnd(')'));
                        //entity.START_TIME = "";
                        entity.START_TIME = $"{Convert.ToDateTime(meter.VerifyDate):yyyy-MM-dd} {BZvalue[i]}";
                        entity.TS_START_TIME = BZvalue[i];
                        entity.TS_REAL_TIME = SJvalue[i];
                        string[] ererr = valueErr[i]?.Split(':');
                        if (ererr != null && ererr.Length == 3)//00:00:00
                        {
                            int.TryParse(ererr[0], out int tmp1);
                            int.TryParse(ererr[1], out int tmp2);
                            int.TryParse(ererr[2], out int tmp3);
                            entity.TS_ERR_CONC_CODE = (tmp1 * 60 * 60 + tmp2 * 60 + tmp3).ToString();
                        }
                        else
                        ...
                        entity.ERR_UP = "+300";
                        entity.ERR_DOWM = "-300";
                        entity.TS_WAY = "01";//读电量/采脉冲
                        entity.PARA_INDEX = (index + 1).ToString();
                        entity.DETECT_ITEM_POINT = (index + 1).ToString();
                        entity.VOLT = GetPCode("meterTestVolt", "100%Un");//meter.MD_UB.ToString();//
                        sql.Add(entity.ToInsertString());
                        index++;
                    }
                }
        ...
        protected string[] GetMT_DEMANDVALUE_MET_CONC(MT_METER mtMeter, TestMeterInfo meter)
        {
            ...
            if ((wc[0] != null && wc[0] != "") || (wc[1] != null && wc[1] != ""))
            {
                ...
                entity.REAL_DEMAND = wc[0]?.Replace(" ", "");             //实际需量
                entity.REAL_PERIOD = "15";
                entity.DEMAND_VALUE_ERR = wc[2];        //需量示值误差
                entity.DEMAND_STANDARD = wc[1]?.Replace(" ", "");         //标准需量
                ...
                entity.VOLTAGE = GetPCode("meterTestVolt", "100%Un");              //电压
            ...
            if ((wc[5] != null && wc[5] != "") || (wc[6] != null && wc[6] != ""))
            {
                ...
                entity.DEMAND_TIME = "1";       // 需量滑差时间
                entity.DEMAND_INTERVAL = "1";                                       //需量滑差次数
                entity.REAL_DEMAND = wc[5]?.Replace(" ", "");         //实际需量
                entity.REAL_PERIOD = "15";
                entity.DEMAND_VALUE_ERR = wc[7];    //需量示值误差   
                entity.DEMAND_STANDARD = wc[6]?.Replace(" ", "");     //标准需量
            ...
            if ((wc[10] != null && wc[10] != "") || (wc[11] != null && wc[11] != ""))
            {
                ...
                entity.DEMAND_TIME = "1";               //需量滑差时间
                entity.DEMAND_INTERVAL = "1";           //需量滑差次数
                entity.REAL_DEMAND = wc[10]?.Replace(" ", "");            //实际需量
                entity.REAL_PERIOD = "15";
                entity.DEMAND_VALUE_ERR = wc[12];       //需量示值误差
                entity.DEMAND_STANDARD = wc[11]?.Replace(" ", "");        //标准需量
        ...
        protected string GetMT_DETECT_RSLTByMt(MT_METER mtMeter, TestMeterInfo meter)
        {
            ...
            e.DETECT_TASK_NO = mtMeter.MT_DATECT_OUT_EQUIP.DETECT_TASK_NO; //检定任务编号
            e.EQUIP_CATEG = "01"; //设备类别
            e.SYS_NO = mtMeter.MT_DATECT_OUT_EQUIP.SYS_NO; //系统编号
            ...
            e.VALUE_CONC_CODE = MisDataHelper.GetDemandValueConclusion(meter); //需量示值误差
