...
namespace CLOU.SocketModule.Sock
{
    ...
    internal class Connection
    {
        ...
        public bool Send(Packet.SendPacket sendPack, Packet.RecvPacket recvPack, string PortName)
        {
            ...
            LogModel.LogFrameInfo Lf = new LogModel.LogFrameInfo();
            Lf.strPortNo = PortName;
            //Lf.strEquipName = sendPack.ToString();
            Lf.strEquipName = "DeviceDriver_Reference_CL3115";
            Lf.strItemName = "";
            Lf.strMessage = sendPack.GetPacketName();

            byte[] vData = sendPack.GetPacketData();

            Lf.sendFrm = new Struct.StSRFrame();
            ...
            if (vData == null)
            {
                Lf.sendFrm.Frame = null;
                Lf.sendFrm.FrameMeaning = "发送数据包为null";
                Lf.sendFrm.FrameTime = DateTime.Now;
                E_CL3115.CL3115.RecordMsg(Lf);
                //Const.GlobalUnit.FrameLog.WriteFrameLog(Lf);
            ...
            Console.WriteLine("SendData:({0}) ", sendPack.GetPacketName());//, BitConverter.ToString(vData, 0));
            DataLoger.WriteLog(DateTime.Now, 0, BitConverter.ToString(vData).Replace("-", ""));
            if (!Send(ref vData, sendPack.IsNeedReturn, sendPack.WaiteTime()))
            {

                //Lf.sendFrm.Frame = vData;
                Lf.sendFrm.FrameMeaning = "发送失败";
                //Lf.sendFrm.FrameTime = DateTime.Now;
                E_CL3115.CL3115.RecordMsg(Lf);
                //Const.GlobalUnit.FrameLog.WriteFrameLog(Lf);
                Monitor.Exit(objPackLock);
            ...
            if (sendPack.IsNeedReturn == true && recvPack != null)
            {
                ...
                DataLoger.WriteLog(DateTime.Now, 1, BitConverter.ToString(vData).Replace("-", ""));
                if (false)
                {
                    //TODO:2018带协议 ，解包
                }
