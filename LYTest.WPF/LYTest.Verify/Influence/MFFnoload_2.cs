...
namespace LYTest.Verify.Influence
{
    ...
    /// 试验时间： 20τ ， τ 为启动试验时间
    public class MFFnoload_2 : VerifyBase
    {
        //磁场源电流为10A
        ...
        public override void Verify()
        {
            ...
            //float delayTime = TheoryTime / 3;

            if (Stop) return;
            MessageAdd("正在初始化误差板...", EnumLogType.提示信息);
            StartWcb(6, 0xff);   //打开误差版计数
            ...
            while (true)
            {
                ...
                if (Stop) break;
                //if (state == 0 && DateTime.Now.Subtract(timeBegin).TotalSeconds > 0)
                //{

                //    InitEquipment2(0);
                //    state = 1;
                //}
                //else if (state == 1 && DateTime.Now.Subtract(timeBegin).TotalSeconds > delayTime)
                //{
                //    InitEquipment2(1);
                //    state = 2;
                //}
                //else if (state == 2 && DateTime.Now.Subtract(timeBegin).TotalSeconds > 2 * delayTime)
                //{
                //    InitEquipment2(2);
                //    state = 3;
                //}
            ...
            ResultDictionary["实际时间(秒)"][6] = DateTime.Now.Subtract(timeBegin).TotalSeconds.ToString("F4");
            if (1 == ReadAndDealData())
            {
            ...
            {
                ResultDictionary["结论"][6] = ConstHelper.合格;
