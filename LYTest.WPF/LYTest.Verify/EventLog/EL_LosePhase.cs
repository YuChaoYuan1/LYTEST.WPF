...
namespace LYTest.Verify.EventLog
{
    ...
    class EL_LosePhase : VerifyBase
    {
        ...
        private int[] ReadEventLogInfo2(Cus_PowerYuanJian yuanJian, string name)
        {
            ...
            MessageAdd("读取断相记录总次数", EnumLogType.提示信息);
            string[] eventTimes = MeterProtocolAdapter.Instance.ReadData(yuanJian.ToString() + "相断相总次数");
            ...
            {
                MessageAdd("断相记录开始时间", EnumLogType.提示信息);
                eventTimeStart = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}相断相产生时刻", 1, yuanJian.ToString()));

                if (Stop) return null;
                MessageAdd(string.Format("读取上【{0:D2}】次【{1}】相断相记录结束时间", 1, yuanJian.ToString()), EnumLogType.提示信息);
                eventTimeEnd = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}相断相结束时刻", 1, yuanJian.ToString()));
            }
            else
            {
                MessageAdd(string.Format("读取上【{0:D2}】次【{1}】断相记录", 1, yuanJian.ToString()), EnumLogType.提示信息);
        ...
        private void StartEventLog(Cus_PowerYuanJian yuanJian)
        {
            ...
            switch (yuanJian)
            {
                case Cus_PowerYuanJian.B:
                    ...
                    msg = "读取电表运行状态字5（B相故障状态）";
                    dataFlag = "电表运行状态字5";
                    PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB * 0.55f, OneMeterInfo.MD_UB, xib, 0, xib * 0.2f, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
                case Cus_PowerYuanJian.C:
                    eventMsg = "C相断相事件";
                    msg = "读取电表运行状态字6（C相故障状态）";
                    dataFlag = "电表运行状态字6";
                    PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB , OneMeterInfo.MD_UB * 0.55f, xib,  xib , 0, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
                default:
                    eventMsg = "A相断相事件";
                    msg = "读取电表运行状态字4（A相故障状态）";
                    dataFlag = "电表运行状态字4";
                    PowerOn(OneMeterInfo.MD_UB * 0.55f, OneMeterInfo.MD_UB, OneMeterInfo.MD_UB , 0, xib, xib,  Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
                    break;
            }
            ...
            if (Stop) return;
            PowerOn(OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, OneMeterInfo.MD_UB, xib, xib, xib, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0");
            WaitTime( "恢复电流, 等待" + eventMsg + "记录产生",65);
        }
        ...
        //    if (Stop) return null;
        //    MessageAdd( string.Format("读取【{0}】断相记录总次数", strPhase), EnumLogType.提示信息);
        //    //string dataFlag = "13" + phase.ToString("D2") + "0001";
        //    string[] eventTimes = MeterProtocolAdapter.Instance.ReadData(string.Format("{0}断相总次数", strPhase));
        ...
        //        if (OneMeterInfo.DgnProtocol.ClassName == "CDLT6452007")
        //        {
        //            MessageAdd( string.Format("读取(上{0}次){1}断相记录开始时间", n, strPhase), EnumLogType.提示信息);
        //            startTime = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}断相发生时刻", n, strPhase));

        //            if (Stop) return null;
        //            MessageAdd( string.Format("读取(上{0}次){1}断相记录结束时间", n, strPhase), EnumLogType.提示信息);
        //            endTime = MeterProtocolAdapter.Instance.ReadData(string.Format("(上{0}次){1}断相结束时刻", n, strPhase));
        //        }
        //        else if (OneMeterInfo.DgnProtocol.ClassName == "CDLT698")
        //        {
        //            MessageAdd( string.Format("读取(上{0}次){1}断相记录", n, strPhase), EnumLogType.提示信息);
        //            //List<string> list = new List<string> { "30030700" };
        ...
        //            for (int i = 0; i < base.MeterNumber; i++)
        //            {
        //                startTime[i] = "00000000000000";
        //                endTime[i] = "00000000000000";
        //                if (!MeterInfo[i].YaoJianYn) continue;
        //                if (dic.ContainsKey(i) && dic[i] != null)
        //                {
        //                    if (dic[i].ContainsKey("20220200"))  // 事件记录序号
        //                        eventTimes[i] = (Convert.ToInt32(dic[i]["20220200"][0]) + 1).ToString(); //由于698没有编程总次数，固以序号表示

        //                    if (dic[i].ContainsKey("201E0200")) //开始时间
        //                        startTime[i] = dic[i]["201E0200"][0].ToString();

        //                    if (dic[i].ContainsKey("20200200")) //结论时间
        ...
        //            if (OneMeterInfo.DgnProtocol.ClassName== "CDLT6452007")
        //            {
        //                if ((chr & 0x80) == 0x80) status = "断相";
        //            }
        //            else if (OneMeterInfo.DgnProtocol.ClassName== "CDLT698")
        //            {
        //                if ((chr & 0x100) == 0x100) status = "断相";
        //            }
        //            if (happen == 1)         //事件发生前
        //            {
        //                ResultDictionary[strPhase + "事件产生前事件状态"][j] = status;
        ...
        //            {
        //                ResultDictionary[strPhase + "事件产生后事件状态"][j] = status;
