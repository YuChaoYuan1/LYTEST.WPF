...
namespace LYTest.MeterProtocol.SocketModule.Sock
{
    ...
    {
        private readonly object objSendLock = new object();
        private readonly object objPackLock = new object();
        ...
        public Connection(int commPort, string szBtl, int MaxWaitMSecond, int WaitSecondsPerByte)
        {
            ...
            {
                MaxWaitSeconds = MaxWaitMSecond,
                WaitSecondsPerByte = WaitSecondsPerByte
            };
        }
        ...
        {
            connection.SetDataReceived(OpenAndClose);

            //  bool ret = recvPack.ParsePacket(vData);
        }
        public List<object> GetOADList()
        {
            return connection.GetOADList();
        }
        ...
        public bool Send(Packet.SendPacket sendPack, Packet.RecvPacket recvPack, string PortName)
        {
            ...
            Log.WriteLog(true);
            //发送数据这里触发事件？
            Trace.WriteLine($"{PortName}_{DateTime.Now:HH:mm:ss ffff} >>>:{BitConverter.ToString(vData).Replace("-", " ")}");
            ...
            if (!Send(ref vData, sendPack.IsNeedReturn ))
            {
                ...
                Monitor.Exit(objPackLock);
                Log.WriteLog(false);
                Trace.WriteLine($"{PortName}_{DateTime.Now:HH:mm:ss ffff} <<<:发送失败");
            ...
            if (sendPack.IsNeedReturn == false)//  && vData.Length < 1
            {
                ...
                Trace.WriteLine($"{PortName}_{DateTime.Now:HH:mm:ss ffff} <<<:不需要回复数据");

                return true;
            }

            if (sendPack.IsNeedReturn && (vData == null || vData.Length < 1))
            {
                Log.RecvMeaning = "无回复数据";
                Monitor.Exit(objPackLock);
                Log.WriteLog(false);
                Trace.WriteLine($"{PortName}_{DateTime.Now:HH:mm:ss ffff} <<<:无回复数据");
            ...
            if (sendPack.IsNeedReturn && recvPack == null)
            {
                ...
                Trace.WriteLine($"{PortName}_{DateTime.Now:HH:mm:ss ffff} <<<:没有传入解析包");
        ...
        /// <returns></returns>
        private bool Send(ref byte[] vData, bool isNeedReturn )
        {
