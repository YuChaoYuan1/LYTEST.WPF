...
using System.Net;
using LYTest.MeterProtocol.SocketModule.Sock;

namespace LYTest.MeterProtocol.SocketModule
{
    /// <summary>
   ...
   public  class SockPool
    {
        ...
        public void SetDataReceived(string szSockName, bool OpenAndClose)
        {
            if (DicSock.ContainsKey(szSockName))
            {
                 DicSock[szSockName].SetDataReceived(OpenAndClose);
            }
        }
        public List<object> GetOADList(string szSockName)
        {
            if (DicSock.ContainsKey(szSockName))
            {
              return  DicSock[szSockName].GetOADList();
            }
            return null;
        }
        /// <summary>
        ...
        public bool Send(string szSockName, Packet.SendPacket sendPacket, Packet.RecvPacket recvPacket)
        {
            ...
            if (dicLock.ContainsKey(szSockName))
            {
                lock (dicLock[szSockName])
                {
                    if (!DicSock.ContainsKey(szSockName))
                    {
                        throw new Exception(string.Format("未找到名为{0}的通信口", szSockName));
                    }
