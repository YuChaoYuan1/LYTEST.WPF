...
using LYTest.Mis.HeBei20.Tables;
...
namespace LYTest.Mis.HeBei20
{
    public class HeBei20 : OracleHelper, IMis
    {
        public HeBei20(string ip, int port, string dataSource, string userId, string pwd, string url)
        {
            this.Ip = ip;
            this.Port = port;
        ...
        public bool Update(TestMeterInfo meterInfo)
        {
            ...
            if (meter == null)
            {
                ...
                //if (meter == null)
                //{
                LogManager.AddMessage("没有查到表信息", EnumLogSource.检定业务日志, EnumLevel.Error);
                return false;
                //}
            }
            ...
            {
                sqlrst = false;
                LogManager.AddMessage("外观检查试验插入出现问题" + ex, EnumLogSource.检定业务日志, Utility.Log.EnumLevel.Error);
            }
            ...
            {
                sqlrst = false;
                LogManager.AddMessage("常数试验插入出现问题" + ex, EnumLogSource.检定业务日志, Utility.Log.EnumLevel.Error);
            }
        ...
        protected string[] GetMT_CONST_MET_CONCByMt(MT_METER mtMeter, TestMeterInfo meter)
        {
            ...
            string[] read_type_fee_codes = new string[] { "1", "2", "3", "5", "4" };//总，尖峰，峰，平，谷
            string[] read_type_fee_name = new string[] { "总", "尖", "峰", "平", "谷" };
            ...
            for (int i = 0; i < needUpCode.Count; i++)
            {
                ...
                MT_CONST_MET_CONC entity = new MT_CONST_MET_CONC
                {
                    ...
                    CONST_ERR = "0",
                    READ_TYPE_CODE = needUpCode.ElementAt(i).Key,
                };
        ...
        protected string GetMT_DAYERR_MET_CONCByMt(MT_METER mtMeter, TestMeterInfo meter)
        {
            ...
            string error = v[0] + "|" + v[1] + "|" + v[2] + "|" + v[3] + "|" + v[4];//前面5次误差
                ...
                if (value.Length >= 3)
                {
                    ...
                    for (int i = 0; i < testvalueone.Length; i++)
                    {
                        ...
                        if (i < BZvalue.Length && i < SJvalue.Length && i < valueErr.Length && para.Length > 1)
                        {
                            ...
                            DateTime.TryParse(BZvalue[i], out DateTime dt);
                            entity.START_TIME = dt.ToString("yyyy-MM-dd HH:mm:ss");
                            entity.TS_START_TIME = dt.ToString("yyyy-MM-dd HH:mm:ss");
                            DateTime.TryParse(SJvalue[i], out dt);
                            entity.TS_REAL_TIME = dt.ToString("yyyy-MM-dd HH:mm:ss");
