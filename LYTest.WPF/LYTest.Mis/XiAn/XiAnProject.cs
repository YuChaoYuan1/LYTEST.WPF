...
namespace LYTest.Mis.XiAn
{
    public class XiAnProject : OracleHelper, IMis
    {
        public IServiceProxy Client = null;
        public Core.DownHelper.XiAnProjectDownHelper downHelpe = new Core.DownHelper.XiAnProjectDownHelper();
        ...
        public Dictionary<string, string> PorjectItmeID = new Dictionary<string, string>();
        ...
        public XiAnProject(string ip, int port, string dataSource, string userId, string pwd, string url)
        {
            this.Ip = ip;
            this.Port = port;
        ...
        {
            //下载电表信息
            Core.DownHelper.XiAnProjectDownHelper downData = (Core.DownHelper.XiAnProjectDownHelper)Data;
            //台体编号, 托盘编号列表，1,true
            ...
            if (!ParsMeterXmlInfo(xml, out Dictionary<string, List<TestMeterInfo>> dicMeterInfo, out string err))
            {
                LogManager.AddMessage("解析xml失败:\r\n" + err);
                return false;
            }

            meter = CalculateMeterId(dicMeterInfo);    //解析表位号
            //解析xml
            ...
            for (int i = 0; i < dicMeterInfo.Count; i++)
            {
                ...
                if (dicMeterInfo.ElementAt(i).Value != null)
                {
                    for (int j = 0; j < dicMeterInfo.ElementAt(i).Value.Count; j++)
                    {
                        ...
                        {
                            //箭头方向
                            dicMeterInfo.ElementAt(i).Value[j].MD_Epitope = dicMeterInfo.ElementAt(i).Value[j].Meter_TrayTotalID <= 3 ? (dicMeterInfo.ElementAt(i).Value[j].Meter_TrayTotalID + 3 * i) : (dicMeterInfo.ElementAt(i).Value[j].Meter_TrayTotalID + 6 * (12 - (i + 1)) + 3 * i);
                        }
                        else
                        {
                            //反方向 
                            dicMeterInfo.ElementAt(i).Value[j].MD_Epitope = dicMeterInfo.ElementAt(i).Value[j].Meter_TrayTotalID <= 3 ? (dicMeterInfo.ElementAt(i).Value[j].Meter_TrayTotalID + 6 * (12 - 1) + 3 * (2 - (i + 1))) : (dicMeterInfo.ElementAt(i).Value[j].Meter_TrayTotalID + 3 * (i - 1));
                        }
            ...
            try
            {
                if (resultFlag == "1")
                {
                    ...
                    foreach (XElement e in doc.Element("DATA").Elements("SCHEME_INFO"))
                    {
                        ...
                        string Name = e.Element("ITEM_NAME").Value; ;
                        int Length = 0;
                        int DecimalPlace = 0;
                        ...
                        switch (ItemName)
                        {
                            case "起动试验":
                                ...
                                PorjectItmeID.Add(ProjectID.起动试验, Itemid);
                            ...
                            case "接线检查":  //RS485
                                ...
                                PorjectItmeID.Add(ProjectID.接线检查_黑龙江, Itemid);
                            ...
                            case "潜动试验":
                                ...
                                PorjectItmeID.Add(ProjectID.潜动试验_黑龙江, Itemid);
                            ...
                            case "常数实验":
                                ...
                                PorjectItmeID.Add(ProjectID.电能表常数试验_黑龙江, Itemid);
                                ...
                                string Type = GetConstantName("2");

                                string Rate = GetRateName("0");//费率
                                SOURCE_PARAMS = TestData[3].Split('/');
                            ...
                            case "日计时":
                                ...
                                PorjectItmeID.Add(ProjectID.日计时误差_黑龙江, Itemid);
                                ...
                                break;
                            case "电能示值组合误差":
                                ID = ProjectID.电能示值组合误差_西安;
                                if (!Schema.ContainsKey(ID))
                                    Schema.Add(ID, new SchemaNode());
                                //< SOURCE_PARAMS > 1 | 1 | 0 | *| 1.0 | *| 1.0Imax | 1.0 | *| *</ SOURCE_PARAMS >
                                //< ITEM_PARAMS > 5 | *| *| 1 / 0.05 | 1 </ ITEM_PARAMS >

                                PorjectItmeID.Add(ProjectID.电能示值组合误差_黑龙江, Itemid);
                                ...
                                if (TestMethod == "1")///如果是取表内时段
                                {
                                    ...
                                    //费率时段 | 是否读取表内时段 | 实验电流 | 时段走电量 | 项目编号
                                    values = JoinValue("", IsAuto, PowerData[6], TestElectricQuantity, Itemid);
                            ...
                            case "通讯性能检测(HPLC)":
                                ...
                                PorjectItmeID.Add(ProjectID.载波通信测试, Itemid);
                            ...
                            case "时钟示值误差":
                                ...
                                PorjectItmeID.Add(ProjectID.时钟示值误差_黑龙江, Itemid);
                            ...
                            case "广播校时":
                                ...
                                PorjectItmeID.Add(ProjectID.GPS对时_黑龙江 + Name, Itemid);
                            ...
                            case "远程清零":
                                ...
                                PorjectItmeID.Add(ProjectID.电量清零_黑龙江, Itemid);
                            ...
                            case "远程密钥更新":
                                ...
                                PorjectItmeID.Add(ProjectID.密钥更新_黑龙江, Itemid);
                                Schema[ID].SchemaNodeValue.Add(Itemid);
                                Schema[ID].SorData = SorData;
                                break;
                            //case "密钥恢复":
        ...
        public bool UpdateALL(TestMeterInfo[] meters, ref float FailureResultRatio)
        {
            ...
            SendEncryptionControl(meters);  //远程控制  /远程拉合闸项目
            SendFKEnPower(meters);      //远程保电
            SendFKWarning(meters);      //远程报警 /报警功能

            SendParamConfirmLastCheck(meters);         //参数验证
            #endregion
        ...
        private void SendFKWarning(TestMeterInfo[] meters)
        {
            xml = $"<PARA>";
            ...
            foreach (var meter in meters)
            {
                ...
                var IsKeyReuslt = meter.MeterFKWarning.Keys.Where(x => x.Contains(ProjectID.报警控制)).ToList();
                ...
                MeterFKWarning meterEW = meter.MeterFKWarning[IsKeyReuslt[0]];
                if (meterEW != null)
                {
                    ...
                    xml += $"<TRIAL_TYPE>569</TRIAL_TYPE>\r\n";
                    xml += $"<ITEM_ID>{meterEW.ITEM_ID}</ITEM_ID>\r\n";//子项目ID
                    ...
                    string WarningOnSata = meterEW.WarningOnSata == "√" ? "成功" : "失败";
                    string WarningOffSata = meterEW.WarningOffSata == "√" ? "成功" : "失败";

                    //报警投入状态|报警解除状态
                    xml += $"<EXTENT_DATA>{WarningOnSata}|{WarningOffSata}|</EXTENT_DATA>\r\n";
                    xml += $"<RECHECK_INDEX>1</RECHECK_INDEX>\r\n";
                    xml += $"<SCHEME_ID>{meterEW.FK_LNG_SCHEME_ID}</SCHEME_ID>\r\n";
                    ...
                    string TrialResult = meterEW.Result == "合格" ? "1" : "0";
        ...
        private void SendFKEnPower(TestMeterInfo[] meters)
        {
            xml = $"<PARA>";
            ...
            foreach (var meter in meters)
            {
                ...
                var IsKeyReuslt = meter.MeterFKEnPower.Keys.Where(x => x.Contains(ProjectID.保电控制)).ToList();
                ...
                MeterFKEnPower meterEP = meter.MeterFKEnPower[IsKeyReuslt[0]];
                if (meterEP != null)
                {
                    xml += "<METER><GUID_ID>000000000000</GUID_ID>\r\n";
                    xml += $"<METER_ID>{meter.MD_Epitope}</METER_ID>\r\n";
                    xml += $"<METER_NO>{meter.MD_BarCode}</METER_NO>\r\n";
                    xml += $"<STAION_ID>{meter.BenthNo}</STAION_ID>\r\n";
                    xml += $"<TRIAL_TYPE>570</TRIAL_TYPE>\r\n";
                    xml += $"<ITEM_ID>{meterEP.ITEM_ID}</ITEM_ID>\r\n";//子项目ID
                    ...
                    //保电投入后保电状态|解除后保电状态
                    xml += $"<EXTENT_DATA>{Convert.ToInt32(meterEP.EnPowerOnState)}|{Convert.ToInt32(meterEP.EnPowerOffState)}|</EXTENT_DATA>\r\n";  
                    xml += $"<RECHECK_INDEX>1</RECHECK_INDEX>\r\n";
                    xml += $"<SCHEME_ID>{meterEP.FK_LNG_SCHEME_ID}</SCHEME_ID>\r\n";
                    xml += $"<TASK_ID>{meter.MD_TaskNo}</TASK_ID>\r\n";
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterEP.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        /// </summary>
        /// <param name="meters"></param>
        private void SendEncryptionControl(TestMeterInfo[] meters)
        {
            xml = $"<PARA>";
            ...
            foreach (var meter in meters)
            {
                ...
                var IsKeyReuslt = meter.MeterEncryptionControl.Keys.Where(x => x.Contains(ProjectID.拉合闸控制)).ToList();
                ...
                MeterEncryptionControl meterEC = meter.MeterEncryptionControl[IsKeyReuslt[0]];
                if (meterEC != null)
                {
                    xml += "<METER><GUID_ID>000000000000</GUID_ID>\r\n";
                    xml += $"<METER_ID>{meter.MD_Epitope}</METER_ID>\r\n";
                    xml += $"<METER_NO>{meter.MD_BarCode}</METER_NO>\r\n";
                    xml += $"<STAION_ID>{meter.BenthNo}</STAION_ID>\r\n";
                    xml += $"<TRIAL_TYPE>559</TRIAL_TYPE>\r\n";
                    xml += $"<ITEM_ID>{meterEC.ITEM_ID}</ITEM_ID>\r\n";//子项目ID
                    ...
                    string SwitchOffState = meterEC.SwitchOffState == "√" ? "成功" : "失败";
                    string SwitchOnState = meterEC.SwitchOnState == "√" ? "成功" : "失败";
                    string type = "RemoteTariff_28";
                    //表类型|拉闸动作|合闸动作
                    xml += $"<EXTENT_DATA>{type}|{SwitchOffState}|{SwitchOnState}|</EXTENT_DATA>\r\n";  
                    xml += $"<RECHECK_INDEX>1</RECHECK_INDEX>\r\n";
                    xml += $"<SCHEME_ID>{meterEC.FK_LNG_SCHEME_ID}</SCHEME_ID>\r\n";
                    xml += $"<TASK_ID>{meter.MD_TaskNo}</TASK_ID>\r\n";
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterEC.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        private void SendCommunicationHPLC(TestMeterInfo[] meters)
        {
            ...
            foreach (var meter in meters)
            {
                ...
                if (meterCommunicationHPLC != null)
                {
                    ...
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterCommunicationHPLC.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        private void SendChipAuthentication(TestMeterInfo[] meters)
        {
            ...
            foreach (var meter in meters)
            {
                ...
                if (meterHPLCIDAuthen != null)
                {
                    ...
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterHPLCIDAuthen.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        private void SendRemotingKeyUpdate(TestMeterInfo[] meters)
        {
            ...
            foreach (var meter in meters)
            {
                ...
                if (meterFK != null)
                {
                    ...
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterFK.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        /// </summary>
        /// <param name="meters"></param>
        private void SendParamConfirmLastCheck(TestMeterInfo[] meters)
        {
            xml = $"<PARA>";
            ...
            foreach (var meter in meters)
            {
                ...
                var IsKeyReuslt = meter.MeterDLTDatas.Keys.Where(x => x.Contains(ProjectID.最后核查)).ToList();
                ...
                foreach (var item in IsKeyReuslt)
                {
                    ...
                    if (meterDLTData != null)
                    {
                        ...
                        xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                        xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                        string TrialResult = meterDLTData.Result == "合格" ? "1" : "0";
                        xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                        xml += "</METER>\r\n";
                    }
        ...
        private void SendClearEnerfy(TestMeterInfo[] meters)
        {
            ...
            foreach (var meter in meters)
            {
                ...
                if (meterClearEnerfy != null)
                {
                    ...
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterClearEnerfy.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        private void SendClockError(TestMeterInfo[] meters)
        {
            ...
            foreach (var meter in meters)
            {
                ...
                if (meterClockError != null)
                {
                    ...
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterClockError.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        private void SendEnergyError(TestMeterInfo[] meters)
        {
            ...
            foreach (var meter in meters)
            {
                ...
                var IsKeyReuslt = meter.MeterEnergyError.Keys.Where(x => x.Contains(ProjectID.电能示值组合误差_西安)).ToList();
                ...
                if (meterEnergyError != null)
                {
                    ...
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterEnergyError.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        private void SendConst(TestMeterInfo[] meters)
        {
            ...
            foreach (var meter in meters)
            {
                ...
                if (meterZZErrors != null)
                {
                    ...
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterZZErrors.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        private void SendPreWiring(TestMeterInfo[] meters)
        {
            ...
            foreach (var meter in meters)
            {
                ...
                if (meterPreWiring != null)
                {
                    ...
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterPreWiring.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        private void SendGPSWillCheckTime(TestMeterInfo[] meters)
        {
            xml = $"<PARA>";
            ...
            foreach (var meter in meters)
            {
                ...
                var IsKeyReuslt = meter.MeterDgns.Keys.Where(x => x.Contains(ProjectID.GPS预校时)).ToList();
                ...
                if (meterDgns != null)
                {
                    ...
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterDgns.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                }
        ...
        private void SendQD(TestMeterInfo[] meters)
        {
            ...
            foreach (var meter in meters)
            {
                ...
                if (meterQd != null)
                {
                    ...
                    xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                    xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                    string TrialResult = meterQd.Result == "合格" ? "1" : "0";
                    xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                    xml += "</METER>\r\n";
                ...
                foreach (var item in IsKeyReuslt)
                {
                    ...
                    if (meterError != null)
                    {
                        ...
                        if (meterError.WCData.Length > 0 || meterError.WCData != "")
                        {
                            ...
                            wc1 = wc[0] == "" ? "999" : float.Parse(wc[0]).ToString("F2");
                            wc2 = wc[1] == "" ? "999" : float.Parse(wc[1]).ToString("F2");
                            wcAvg = wc[2] == "" ? "999" : float.Parse(wc[2]).ToString("F2");
                            wcHz = wc[3] == "" ? "999" : wc[3];

                        }

                        if (float.Parse(wc1) > 0.00f)
                        {
                            wc1 = "+" + wc1;
                        }
                        else if (float.Parse(wc1) == 0.00f)
                        {
                            wc1 = "-" + wc1;
                        }

                        if (float.Parse(wc2) > 0.00f)
                        {
                            wc2 = "+" + wc2;
                        }
                        else if (float.Parse(wc2) == 0.00f)
                        {
                            wc2 = "-" + wc2;
                        }

                        if (float.Parse(wcAvg) > 0.00f)
                        {
                            wcAvg = "+" + wcAvg;
                        }
                        else if (float.Parse(wcAvg) == 0.00f)
                        {
                            wcAvg = "-" + wcAvg;
                        }

                        if (float.Parse(wcHz) > 0.00f)
                        {
                            wcHz = "+" + wcHz;
                        }
                        else if (float.Parse(wcHz) == 0.00f)
                        {
                            wcHz = "-" + wcHz;
                        }
                        ...
                        xml += $"<PRISTINE_VALUES>{wc1},{wc1},</PRISTINE_VALUES>\r\n";//原始值
                        xml += $"<TRIAL_VALUE>{wcAvg}</TRIAL_VALUE>\r\n"; //平均值
                        xml += $"<ROUND_VALUE>{wcHz}</ROUND_VALUE>\r\n";//化整值
                        ...
                        xml += $"<TRIAL_ENDDATE>{DateTime.Now}</TRIAL_ENDDATE>\r\n";
                        xml += $"<TRIAL_STARTDATE>{DateTime.Now}</TRIAL_STARTDATE>\r\n";
                        string TrialResult = meterError.Result == "合格" ? "1" : "0";
                        xml += $"<TRIAL_RESULT>{TrialResult}</TRIAL_RESULT>\r\n";
                        xml += "</METER>\r\n";
                        //}
        ...
        public Dictionary<string, SchemaNode> SortScheme(Dictionary<string, SchemaNode> Schema)
        {
            ...
            if (Schema.ContainsKey(ProjectID.GPS预校时))
                TemScheme.Add(ProjectID.GPS预校时, Schema[ProjectID.GPS预校时]);
            ...
            if (Schema.ContainsKey(ProjectID.电能示值组合误差_西安))
                TemScheme.Add(ProjectID.电能示值组合误差_西安, Schema[ProjectID.电能示值组合误差_西安]);
            ...
            if (Schema.ContainsKey(ProjectID.拉合闸控制))
                TemScheme.Add(ProjectID.拉合闸控制, Schema[ProjectID.拉合闸控制]);
            if (Schema.ContainsKey(ProjectID.保电控制))
                TemScheme.Add(ProjectID.保电控制, Schema[ProjectID.保电控制]);
            if (Schema.ContainsKey(ProjectID.报警控制))
                TemScheme.Add(ProjectID.报警控制, Schema[ProjectID.报警控制]);
            ...
            if (Schema.ContainsKey(ProjectID.最后核查))
                TemScheme.Add(ProjectID.最后核查, Schema[ProjectID.最后核查]);
            return TemScheme;
