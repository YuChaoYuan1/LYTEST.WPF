...
namespace LYTest.Verify.AccurateTest
{
    ...
    public class DeviationModel : VerifyBase
    {
        ...
        public override void Verify()
        {
            ...
            if (!PowerOn(OneMeterInfo.MD_UB * SetUn, OneMeterInfo.MD_UB * SetUn, OneMeterInfo.MD_UB * SetUn, 0, 0, 0, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0"))
            {
        ...
        private void CalculationProcess(string name, int WaitTimeSecond, string DecimalPlaces)
        {
            ...
            LoadResultDictionary("间隔标准值", EquipmentData.StdInfo.Ub, DecimalPlaces);
            LoadResultDictionary("模组电压偏差相对误差", EquipmentData.StdInfo.Ub - MeasurementValue, DecimalPlaces);
        }
