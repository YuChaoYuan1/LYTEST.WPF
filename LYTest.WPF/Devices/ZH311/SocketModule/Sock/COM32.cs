    ...
    internal class COM32 : IConnection
    {
        ...
        public bool SendData(ref byte[] vData, bool IsReturn,int WaiteTime)
        {
            lock (ThreadObject)
            {
                ...
                //Console.WriteLine("┏【标准表】SendData:{0}", BitConverter.ToString(vData));
                spCom.WriteTimeout = MaxWaitSeconds;
                try
                {
                    spCom.Write(vData, 0, vData.Length);
                    //Console.WriteLine("┗发送【标准表】数据成功===========================");
                }
                ...
                if (!IsReturn)
                {
                    //Console.WriteLine("┗【标准表】关闭串口===========================");
                    ////spCom.Close();
                    //Console.WriteLine("┗【标准表】本包不需要回复");
                    return true;     //如果不需要返回
                }
                bool IsOut = false;
                ...
                if (IsOut)      //如果超时就将需要返回的数组数量设置为0
                {
                    vData = new byte[0];
                    //spCom.Close();
                    //Console.WriteLine("┗【标准表】RecvData:接收超时");    
                    return true;
                }
              ...
              //  Console.WriteLine("┗【标准表】RecvData:{0}", BitConverter.ToString(vData));
                //this.Close();
                return true;
            }
        }
