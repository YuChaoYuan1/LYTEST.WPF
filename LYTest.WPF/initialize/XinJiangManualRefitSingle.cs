...
{
    class XinJiangManualRefitSingle : Universal
    {
        public override void Execute()
        {
            try
            {
                ...
                {
                    Add_T_CONFIG_PARA_VALUE("01001", "1|单相台|60|电能表|手动模式|否|是|是|正转");
                }
                else
                {
                    ...
                    if (cfgs.Length > 2)
                    {
                        Add_T_CONFIG_PARA_VALUE("01001", $"{cfgs[0]}|单相台|{cfgs[2]}|电能表|手动模式|否|是|是|正转");
                    }
                }

                //Add_T_CONFIG_PARA_VALUE("02007", "是|15|否");//自动压接|压接等待时间|直接互感自动切换
...
//, "新疆生产调度平台|条形码|10.218.15.201|11521|pmcpdp|sxykjd|djkyxs_121#|http://10.218.163.113:17011/InterfaceWS/InterfaceBusiness/services/DetectService?wsdl|否|10.218.15.201|11521|pmcpdp|sxykjd|djkyxs_121#|否");
                ...
                Add_T_CODE_TREE("E_CL188M", "", "4", "ErrorBoard");
            }
            catch (Exception ex)
            {
                ShowMsg(ex.Message);
